...
$.fn.generaMenu = function(menu) {
   this.each(function(){
      var retardo;
      var capaMenu = $(this);      
      var listaPrincipal = $('<ul class="nav nav-pills nav-justified"></ul>');
      ...
      {         
         ...
         if(this.url=="#")
         {
             enlacePrincipal = $('<a title="'+this.texto+'">' + this.texto + '<i class="fa fa-angle-down fa-fw"></i></a>');
         }
         else 
         {
             enlacePrincipal = $('<a title="'+this.texto+'" href="' + this.url + '">' + this.texto + '<i class="fa fa-angle-down fa-fw"></i></a>');
         }
         ...
         function colocarCapasSubmenus(){
            $.each(arrayCapasSubmenu, function(i){
               ...
               var wmp = arrayLiMenuPrincipal[i].width();               
               this.css({
                  left: posicionEnlace.left,
                  top: posicionEnlace.top + 48,
                  width:wmp,
                  marginLeft:1
               });
            });
