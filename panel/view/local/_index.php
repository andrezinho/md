...
$(document).ready(function()
{
  $("#qry").focus();
  $("#list").jqGrid({
      url:'index.php?controller=<?php echo $controlador; ?>&emp=<?php echo $idempresa; ?>&action=indexGrid',
      datatype: "json",
      colNames:cNames,
      colModel:cModel,
      rowNum:10,
...
function gridReload()
{
  var fltr = $("#fltr").val(),
      qry = $("#qry").val(),
      emp = $("#empre").val();
  $("#list").jqGrid('setGridParam',{url:"index.php?controller=<?php echo $controlador; ?>&emp="+emp+"&action=indexGrid&f="+fltr+"&q="+qry,page:1}).trigger("reloadGrid");
}
...
function nuevo()
{
    $("#loader").css("display","inline-block");
    var emp = $("#empre").val();
    $.get('index.php?controller=<?php echo $controlador; ?>&action=create&emp='+emp,function(html)
    {
       $.getScript("js/app/evt_form_<?php echo strtolower($controlador); ?>.js");
       $("#box-frm").dialog({buttons: {
                  'Cancelar': function(){ $(this).dialog('close');},
  ...
  <div style="padding:10px;">
    <div style="padding:10px; border-bottom:0 " class="ui-widget-content ui-corner-top">
        ...
        <div style="padding:10px 0 0px; ">
              ...
              <?php echo $cmb_search; ?>
              <input type="hidden" name="empre" id="empre" value="<?php echo $idempresa ?>" />
...
<div class="box-lateral-option">
    ...
    <div>
        <ul class="enlaces-menu">
            ...
            <li><a href="#" class="logout">Cambiar Contraseña</a></li>            
