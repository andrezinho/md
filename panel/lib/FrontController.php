...
class FrontController 
{
    public static function Main() 
    {
        ...
        if( empty( $action ) )     
            $action = "index";    
        if (!isset($_SESSION['idusuario']) && empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) != 'xmlhttprequest'  )     
            header('Location: login.php');
        if( !empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' &&!isset ($_SESSION['idusuario'])   ) 
        {
