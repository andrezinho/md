...
class Main extends Spdo {
    ...
    public function execQuery($page,$limit,$sidx,$sord,$filtro,$query,$cols,$sql)
    {
        ...
        if($filtro!="") 
        {
            ...
            $sql .= " cast(".$filtro." as nchar) like :query ";
        }
    ...
    */
    ...
    //Valida si la cadena es una Fecha: 
        ...
        {
            $sth = $this->db->prepare("SELECT idubigeo,descripcion,substr(idubigeo,3,4) from ubigeo where substr(idubigeo,3,4)='0000'");
            $sth->execute();
            $data = array();
            foreach ($sth->fetchAll() as $key => $value) {
                $data[] = array($value['idubigeo'],$value['descripcion']);
            }
            return $data;
        }
        
        public function provincias($cod)
        {
            $sth = $this->db->prepare("SELECT idubigeo,descripcion,substr(idubigeo,5,2)
                                       from ubigeo where  substr(idubigeo,1,2) like '".$cod."%' and substr(idubigeo,5,2)='00' and substr(idubigeo,3,4)<>'0000'");
            $sth->execute();
            $data = array();
            foreach ($sth->fetchAll() as $key => $value) 
            {
        ...
        public function distritos($cod)
        {
            $sth = $this->db->prepare("SELECT idubigeo,descripcion,substr(idubigeo,1,4)
                                        from ubigeo where  substr(idubigeo,1,4) like '".$cod."%' and substr(idubigeo,5,2)<>'00'");
            $sth->execute();
            $data = array();
            foreach ($sth->fetchAll() as $key => $value) {
                $data[] = array($value['idubigeo'],$value['descripcion']);
